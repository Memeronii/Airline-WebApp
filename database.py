...
def delete_user(userid):
    ...
    try:
        ...
        conn.close()                    # Close the connection to the db
        raise
def list_aircraft(sort_by='AircraftID', sort_order='asc'):
    conn = database_connect()
    if conn is None:
        return None
    cur = conn.cursor()
    ...
    valid_columns = ['AircraftID', 'ICAOCode', 'AircraftRegistration', 'Name', 'Manufacturer', 'Model']
    ...
    try:
        sql = f"""SELECT AircraftID, ICAOCode, AircraftRegistration, Name, Manufacturer, Model 
                  FROM aircraft 
                  ORDER BY {sort_by} {sort_order}"""
        cur.execute(sql)
        columns = [col[0] for col in cur.description]
        returndict = [dict(zip(columns, row)) for row in cur.fetchall()]
        print("Fetched aircraft data:", returndict)  
    except Exception as e:
        import traceback
        traceback.print_exc()
...
def get_aircraft_by_id(aircraft_id):
    ...
    cur = conn.cursor()
    try:
        sql = """SELECT AircraftID, ICAOCode, AircraftRegistration, Name, Manufacturer, Model FROM aircraft WHERE AircraftID = %s"""
        cur.execute(sql, [aircraft_id])
        row = cur.fetchone()
        if row is None:
            print(f"No aircraft found with ID {aircraft_id}")
            return None
        columns = [col[0] for col in cur.description]
        aircraft = dict(zip(columns, row))
        print("Fetched aircraft data by ID:", aircraft) 
        return aircraft
    except Exception as e:
        print("Error fetching aircraft by ID:", e)
        return None
    finally:
        cur.close()
        conn.close()
def add_aircraft_insert(aircraft):
    ...
    sql = """
        ...
        VALUES (%s, %s, %s, %s, %s, %s);
        """
    print_sql_string(sql, (aircraft['AircraftID'], aircraft['ICAOCode'], aircraft['AircraftRegistration'], aircraft['Name'], aircraft['Manufacturer'], aircraft['Model']))
    try:
        # Try executing the SQL and get from the database

        cur.execute(sql, (aircraft['AircraftID'], aircraft['ICAOCode'], aircraft['AircraftRegistration'], aircraft['Name'], aircraft['Manufacturer'], aircraft['Model']))
        ...
        # If there were any errors, return a NULL row printing an error to the debug
        print("Unexpected error adding an aircraft:", sys.exc_info()[0])
        cur.close()                     # Close the cursor
        conn.close()                    # Close the connection to the db
        raise

def update_aircraft_by_id(aircraft_id, aircraft):
    conn = database_connect()
    if conn is None:
        return None
    cur = conn.cursor()
    try:
        sql = """
        UPDATE aircraft
        SET AircraftID = %s, ICAOCode = %s, AircraftRegistration = %s, Name = %s, Manufacturer = %s, Model = %s
        WHERE AircraftID = %s
        """
        cur.execute(sql, [aircraft['AircraftID'], aircraft['ICAOCode'], aircraft['AircraftRegistration'], aircraft['Name'], aircraft['Manufacturer'], aircraft['Model'], aircraft_id])
        conn.commit()
    except Exception as e:
...
def get_aircraft_stats():
    ...
    try:
        ...
        """
        stats = dictfetchall(cur, sql)
        return stats if stats else []
    except Exception as e:
        print("Error fetching aircraft stats:", e)
        return []
    finally:
        cur.close()
        conn.close()
def search_aircraft(filters):
    conn = database_connect()
    if conn is None:
        return None
    cur = conn.cursor()
    ...
    try:
        sql = """SELECT AircraftID, ICAOCode, AircraftRegistration, Name, Manufacturer, Model 
                 FROM aircraft WHERE 1=1"""
        params = []
        if filters['AircraftID']:
            sql += " AND AircraftID = %s"
            params.append(filters['AircraftID'])
        if filters['ICAOCode']:
            sql += " AND ICAOCode ILIKE %s"
            params.append(f"%{filters['ICAOCode']}%")
        if filters['AircraftRegistration']:
            sql += " AND AircraftRegistration ILIKE %s"
            params.append(f"%{filters['AircraftRegistration']}%")
        if filters['Name']:
            sql += " AND Name ILIKE %s"
            params.append(f"%{filters['Name']}%")
        if filters['Manufacturer']:
            sql += " AND Manufacturer ILIKE %s"
