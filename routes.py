...
def list_aircraft_route():
    sort_by = request.args.get('sort_by', 'AircraftID')
    sort_order = request.args.get('sort_order', 'asc')
    aircraft_listdict = database.list_aircraft(sort_by, sort_order)
    if aircraft_listdict is None:
        aircraft_listdict = []
        flash('Error, there are no rows in aircraft')
    else:
        if len(aircraft_listdict) > 0:
            print("Keys in the aircraft dic:", aircraft_listdict[0].keys())
    
    page = {'title': 'List of Aircraft'}
    print("Aircraft data passed to template:", aircraft_listdict) 
    return render_template('list_aircraft.html', page=page, session=session, aircraft=aircraft_listdict, sort_by=sort_by, sort_order=sort_order)

@app.route('/aircraft/<aircraft_id>')
def show_aircraft(aircraft_id):
    aircraft = database.get_aircraft_by_id(aircraft_id)
    if aircraft is None:
        flash(f"No aircraft found with ID {aircraft_id}")
        return redirect(url_for('list_aircraft_route'))
    page['title'] = f'Aircraft Details for ID {aircraft_id}'
    return render_template('show_aircraft.html', page=page, session=session, aircraft=aircraft)

@app.route('/aircraft/search', methods=['GET', 'POST'])
def search_aircraft():
    page = {'title': 'Aircraft Search'}
    if request.method == 'POST':
        filters = {
            'AircraftID': request.form.get('AircraftID'),
            'ICAOCode': request.form.get('ICAOCode'),
            'AircraftRegistration': request.form.get('AircraftRegistration'),
            'Name': request.form.get('Name'),
            'Manufacturer': request.form.get('Manufacturer'),
            'Model': request.form.get('Model')
        }
        aircraft_listdict = database.search_aircraft(filters)
        if aircraft_listdict is None:
            aircraft_listdict = []
            flash('No aircraft found matching the search criteria')
        return render_template('list_aircraft.html', page=page, session=session, aircraft=aircraft_listdict)
    return render_template('search_aircraft.html', page=page, session=session)

@app.route('/aircraft/add', methods=['GET', 'POST'])
def add_aircraft():
    page = {'title': 'Add Aircraft'}
    if request.method == 'POST':
        try:
            aircraft_id = int(request.form['AircraftID'])
        except ValueError:
            flash('Please enter an integer for Aircraft ID', 'danger')
            return render_template('add_aircraft.html', page=page, session=session)

        try:
            model = int(request.form['Model'])
        except ValueError:
            flash('Please enter an integer for Capacity', 'danger')
            return render_template('add_aircraft.html', page=page, session=session)

        existing_aircraft = database.get_aircraft_by_id(aircraft_id)
        if existing_aircraft:
            flash('Duplicate Aircraft ID - Please enter a unique Aircraft ID', 'danger')
            return render_template('add_aircraft.html', page=page, session=session)

        new_aircraft = {
            'AircraftID': aircraft_id,
            'ICAOCode': request.form['ICAOCode'],
            'AircraftRegistration': request.form['AircraftRegistration'],
            'Name': request.form['Name'],
            'Manufacturer': request.form['Manufacturer'],
            'Model': model
        }
        database.add_aircraft_insert(new_aircraft)
        flash('Aircraft added successfully', 'success')
        return redirect(url_for('list_aircraft_route'))
    return render_template('add_aircraft.html', page=page, session=session)
@app.route('/aircraft/update/<int:aircraft_id>', methods=['GET', 'POST'])
def update_aircraft(aircraft_id):
    page = {'title': f'Update Aircraft {aircraft_id}'}
    if request.method == 'POST':
        try:
            new_aircraft_id = int(request.form['AircraftID'])
        except ValueError:
            flash('Please enter an integer for Aircraft ID', 'danger')
            return render_template('update_aircraft.html', aircraft=database.get_aircraft_by_id(aircraft_id), session=session, page=page)
        ...
        existing_aircraft = database.get_aircraft_by_id(new_aircraft_id)
        if existing_aircraft and new_aircraft_id != aircraft_id:
            flash('Duplicate Aircraft ID - Please enter a unique Aircraft ID', 'danger')
            return render_template('update_aircraft.html', aircraft=database.get_aircraft_by_id(aircraft_id), session=session, page=page)
        ...
        database.update_aircraft_by_id(aircraft_id, updated_aircraft)
        flash('Aircraft updated successfully', 'success')
        return redirect(url_for('list_aircraft_route'))
    aircraft = database.get_aircraft_by_id(aircraft_id)
    if aircraft is None:
        flash(f"No aircraft found with ID {aircraft_id}", 'danger')
        return redirect(url_for('list_aircraft_route'))
    return render_template('update_aircraft.html', aircraft=aircraft, session=session, page=page)

@app.route('/aircraft/delete/<int:aircraft_id>', methods=['POST'])
def delete_aircraft(aircraft_id):
    database.delete_aircraft_by_id(aircraft_id)
    flash('Aircraft deleted successfully')
    return redirect(url_for('list_aircraft_route'))

@app.route('/aircraft/stats')
def aircraft_stats():
    stats = database.get_aircraft_stats()
    if stats is None:
        flash('Error fetching aircraft stats')
        stats = []
    page = {'title': 'Aircraft Stats'}
    return render_template('aircraft_stats.html', stats=stats, page=page, session=session)

@app.route('/aircraft/compare', methods=['GET', 'POST'])
def compare_aircraft():
    page = {'title': 'Compare Aircraft'}
    aircraft1 = None
    aircraft2 = None
    if request.method == 'POST':
        aircraft_id1 = request.form.get('AircraftID1')
        aircraft_id2 = request.form.get('AircraftID2')
        if not aircraft_id1 or not aircraft_id2:
            flash('Both Aircraft IDs are required')
            return redirect(url_for('compare_aircraft'))
        aircraft1 = database.get_aircraft_by_id(aircraft_id1)
        aircraft2 = database.get_aircraft_by_id(aircraft_id2)
        if not aircraft1 or not aircraft2:
            flash('One or both Aircraft IDs are invalid')
            return redirect(url_for('compare_aircraft'))
    return render_template('compare_aircraft.html', page=page, session=session, aircraft1=aircraft1, aircraft2=aircraft2)